...
public class AuthTest {
    ...
    public void setUp() throws Exception {
        sdk = DnaSdk.getInstance();
        sdk.setRestful(DnaSdkTest.URL);
        ...
        password = "111111";
        adminIdentity  = sdk.getWalletMgr().createIdentityFromPriKey(password,DnaSdkTest.PRIVATEKEY);
        identity  = sdk.getWalletMgr().createIdentityFromPriKey(password,DnaSdkTest.PRIVATEKEY2);
        identity2 = sdk.getWalletMgr().createIdentityFromPriKey(password,DnaSdkTest.PRIVATEKEY3);
    ...
    public void test() throws Exception {
        sdk.nativevm().dnaId().sendRegister(adminIdentity,password,account,sdk.DEFAULT_GAS_LIMIT,0);
        sdk.nativevm().dnaId().sendRegister(identity,password,account,sdk.DEFAULT_GAS_LIMIT,0);
        sdk.nativevm().dnaId().sendRegister(identity2,password,account,sdk.DEFAULT_GAS_LIMIT,0);
    ...
    public void sendTransfer() throws Exception {
        String txhash = sdk.nativevm().auth().sendTransfer(identity.dnaid,password,identity.controls.get(0).getSalt(),1,codeAddress,adminIdentity.dnaid,account,sdk.DEFAULT_GAS_LIMIT,0);
        Thread.sleep(6000);
    ...
    public void assignFuncsToRole2() throws Exception {
        ...
        String txhash2 = sdk.nativevm().auth().assignFuncsToRole(adminIdentity.dnaid, password, adminIdentity.controls.get(0).getSalt(), 1,
        ...
        assertTrue(((JSONObject) obj).getString("State").equals("1"));
        txhash = sdk.nativevm().auth().assignDnaIdsToRole(adminIdentity.dnaid, password, adminIdentity.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "role1", new String[]{identity2.dnaid}, account, sdk.DEFAULT_GAS_LIMIT, 0);
        Thread.sleep(6000);
        obj = sdk.getConnect().getSmartCodeEvent(txhash);
        assertTrue(((JSONObject) obj).getString("State").equals("1"));
        String result = sdk.nativevm().auth().verifyToken(identity2.dnaid, password, identity2.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "foo1");
    ...
    public void assignFuncsToRole() throws Exception {
        String txhash = sdk.nativevm().auth().assignFuncsToRole(adminIdentity.dnaid, password, adminIdentity.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "role", new String[]{"foo"}, account, sdk.DEFAULT_GAS_LIMIT, 0);
        Thread.sleep(6000);
        Object obj = sdk.getConnect().getSmartCodeEvent(txhash);
        assertTrue(((JSONObject) obj).getString("State").equals("1"));
        txhash = sdk.nativevm().auth().assignDnaIdsToRole(adminIdentity.dnaid, password, adminIdentity.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "role", new String[]{identity2.dnaid}, account, sdk.DEFAULT_GAS_LIMIT, 0);
        Thread.sleep(6000);
        obj = sdk.getConnect().getSmartCodeEvent(txhash);
        assertTrue(((JSONObject) obj).getString("State").equals("1"));
        String result = sdk.nativevm().auth().verifyToken(identity2.dnaid, password, identity2.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "foo");
        assertTrue(result.equals("01"));
    }

    @Test
    public void delegate() throws Exception {
        sdk.nativevm().auth().delegate(identity2.dnaid,password,identity2.controls.get(0).getSalt(),1,Helper.reverse(codeAddress),identity.dnaid,"role",60*5,1,account,sdk.DEFAULT_GAS_LIMIT,0);
        Thread.sleep(6000);
        sdk.nativevm().auth().withdraw(identity2.dnaid,password,identity2.controls.get(0).getSalt(),1,Helper.reverse(codeAddress),identity.dnaid,"role1",account,sdk.DEFAULT_GAS_LIMIT,0);
        String result = sdk.nativevm().auth().verifyToken(identity2.dnaid, password, identity2.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "foo");
        String result2 = sdk.nativevm().auth().verifyToken(identity.dnaid, password, identity.controls.get(0).getSalt(), 1, Helper.reverse(codeAddress), "foo");
